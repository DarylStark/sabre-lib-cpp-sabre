...
{
    int var = 0;
    sabre::impl::sabre_testing::StService my_service([&var]() { ++var; });
    my_service.start();
...
{
    int var = 0;
    sabre::impl::sabre_testing::StService my_service(
...
TEST(Service, StartingWithAFunctor)
{
    ...
    sabre::impl::sabre_testing::StService my_service(std::ref(fn));
    my_service.start();
    ASSERT_EQ(fn.var, 1);
}